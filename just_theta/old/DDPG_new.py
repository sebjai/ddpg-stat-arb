...
class DDPG():
    ...
    def Update_Q(self, mini_batch_size=256, epsilon=0.02, n_iter_Q=1):
        ...
        for i in range(n_iter_Q):
            ...
            S_gru_p, y = self.create_snippets(batch_S[:, self.seq_length+self.n_ahead+1].T)

            S_p = self.gru['net'](S_gru_p.transpose(0,2))#
    ...
    def run_strategy(self, name= datetime.now().strftime("%H_%M_%S"), N = 12, no_plots = False):
        ...
        for t in range(N-self.seq_length):
            ...
            S_gru, y = self.create_snippets(S[:, t+self.seq_length-1].T)

            S_ = self.gru['net'](S_gru.transpose(0,2)).detach()

            X = self.__stack_state__(S_, I[:, t+self.seq_length-1].T)
