...
class MR_env():
    ...
    def Randomize_Start(self, mini_batch_size=10):
        ...
        S, _, theta = self.Simulate(self.S_0 + 3*self.inv_vol*torch.randn(mini_batch_size, ),
                             0, model = 'MC', mini_batch_size = mini_batch_size)
        I = self.I_max * (2*torch.rand(mini_batch_size, self.N)-1)
        
        return S, I, theta

    def Simulate(self, s0, i0, model = 'exp', mini_batch_size=10):
        ...
        if model == 'exp':
            ...
            for t in range(1, self.N-1):
                for j in range(mini_batch_size):
                    theta[j,t] = torch.tensor(np.random.choice(states, p = np.round(probabilities[np.where(states == theta[j, t-1])][0], 5)))#states[torch.choose(torch.tensor(probabilities), mini_batch_size)]
