...
class gru_pred():
    ...
    def find_normalization(self, s, batch_size=1_000):
    ...
    def handle_y(self, y):
        y_i = torch.zeros(y.shape[0], len(self.env.theta)).to(self.device)
    ...
    def train(self, num_epochs=10_000, n_print=100):
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            #print("Targets:", y)
            #target_indices = self.handle_y(y)
...
#
    ...
    def pred(self):
        ...
        lg = nn.Softmax(dim=1)
        pre = self.model(x).detach().squeeze()
        pred = lg(pre).cpu().numpy()
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax2 = ax.twinx()

        #x = x*self.stdde + self.media
        
        ax.plot(t, x[:, -1, 0].cpu(), label=r'$S_t$', color='k')
        for k in range(len(self.env.theta)):
            ax2.plot(t, pred[:, k], label=r"$\widehat{\mathbb{P}}[\theta_{t+n} = \theta^{(" + str(k) + ")}|\mathcal{F}_t]$", alpha=0.5)
            
        ax.plot(t, y.cpu(), label=r'$\theta_{t}$', color='tab:red')
