...
#
    ...
    def Update_Q(self, mini_batch_size=256, epsilon=0.02, n_iter_Q=1):
        ...
        for i in range(n_iter_Q):
            ...
            Q = self.Q_main['net'].to(device)( torch.cat((X, I_p/self.I_max),axis=-1) )
            ...
            target = r + self.gamma * self.Q_target['net'].to(device)(torch.cat((X_p, I_pp/self.I_max), axis=1))
    ...
    def run_strategy(self, name= datetime.now().strftime("%H_%M_%S"), N = 12, no_plots = False):
        ...
        S = torch.zeros((1, N+2), device= device).float()         
        I = torch.zeros((1, N+2), device= device).float()
