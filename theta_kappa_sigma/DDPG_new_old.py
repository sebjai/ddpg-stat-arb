...
class DDPG():
    ...
    def __stack_state__(self, S, I):
        ...
        return torch.cat((
            (S/self.env.S_0-1.0), #S.unsqueeze(-1),#
            (I/self.I_max  ), 
            ), axis=0)
    ...
    def Update_Q(self, mini_batch_size=256, epsilon=0.02, n_iter_Q=1):
        ...
        for i in range(n_iter_Q):
            ...
            I_p = self.pi['net'](X.transpose(0,2)).detach() * torch.exp(-0.5*epsilon**2+epsilon*torch.randn(batch_S.shape[0],1))

            I_q = self.create_snippets(I_p)[0].transpose(0,2)

            Q = self.Q_main['net'](torch.cat((batch_S[:, self.seq_length+1].unsqueeze(-1), batch_I[:, self.seq_length+1].unsqueeze(-1), I_p/self.I_max),axis=-1))
            ...
            I_pp = self.pi['net'](X_p.transpose(0,2)).detach()

            # compute the target for Q
            target = r + self.gamma * self.Q_target['net'](torch.cat((batch_S[:,self.seq_length+2].unsqueeze(-1), batch_I[:, self.seq_length+2].unsqueeze(-1), I_pp/self.I_max),axis=-1))
    ...
    def Update_pi(self, mini_batch_size=256, epsilon=0.02, n_iter_pi=10):
        ...
        for i in range(n_iter_pi):
            ...
            I_p = self.pi['net'](X.transpose(0,2))
    ...
    def run_strategy(self, name= datetime.now().strftime("%H_%M_%S"), N = 12, no_plots = False):
        ...
        for t in range(N-self.seq_length-1):
            ...
            I[:, t+self.seq_length+1] = self.pi['net'](X.transpose(0,2)).reshape(-1).detach().unsqueeze(-1)
